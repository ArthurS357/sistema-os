...
const App: React.FC = () => {
  ...
  const saveToDisk = async (newDb: Database) => {
    ...
    if (!result.success) toast.error("Falha ao salvar!");
  };
  ...
  const handleSyncFiles = async () => {
    ...
    } catch (e) { toast.dismiss(toastId); toast.error("Erro na sincronização."); }
  };
    ...
    if (print) {
      ...
      try {
        const result = await window.api.generateDocx({
          os: currentId!, data: dataHoje, cliente: form.cliente, impressora: form.impressora,
          orcamento: form.orcamento, valor: form.valor, status: form.status, obs: obsFinal
        });
        toast.dismiss(toastId);
        if (result.success) { window.print(); toast.success(`O.S. ${currentId} impressa!`); }
        else toast.error("Erro Word: " + result.error);
